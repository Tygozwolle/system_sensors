...
def get_host_model():
    if "rasp" in OS_DATA["ID"] and isDockerized and isDeviceTreeModel:
        model = subprocess.check_output(["cat", "/app/host/proc/device-tree/model"]).decode("UTF-8").strip() # nosec
...
def on_message(client, userdata, message):
    ...
    if message.payload.decode() == 'online':
        ...
        reading = subprocess.check_output([vcgencmd, "display_power", "1"]).decode("UTF-8") # nosec
        update_sensors()
